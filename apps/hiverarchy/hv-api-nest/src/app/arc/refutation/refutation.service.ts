...
export class RefutationService {
  constructor(@InjectModel(Refutation.name) private refutationModel: Model<RefutationDocument>) {}
  ...
  async updateRefutation(id: string, updateRefutationDto: any): Promise<Refutation> {
    return this.refutationModel
      .findByIdAndUpdate(id, updateRefutationDto, { new: true })
      .exec();
  }

  async delete(id: string): Promise<Refutation> {
    return this.refutationModel.findByIdAndRemove(id).exec();
  }
}
