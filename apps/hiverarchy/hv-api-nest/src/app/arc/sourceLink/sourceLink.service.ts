...
export class SourceLinkService {
  constructor(@InjectModel(SourceLink.name) private sourceLinkModel: Model<SourceLinkDocument>) {}
  ...
  async updateSourceLink(id: string, updateSourceLinkDto: any): Promise<SourceLink> {
    return this.sourceLinkModel
      .findByIdAndUpdate(id, updateSourceLinkDto, { new: true })
      .exec();
  }

  async delete(id: string): Promise<SourceLink> {
    return this.sourceLinkModel.findByIdAndRemove(id).exec();
  }
}
